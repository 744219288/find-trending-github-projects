...
description: >-
  Discover and analyze the latest popular GitHub repositories. Supports three
  modes: GitHub Trending cards, keyword-driven deep reports, and subscription
  deltas. Use for current, trending, fast-growing, newly released, keyword Top
  5–20, or repeatable snapshot requests. Produces a self-contained HTML report
  using official GitHub data and resolves the launch contract before collecting.
---
...
- Output: self-contained card-style HTML report (`report.html`, offline-openable) — **默认只生成这一个文件**；如需 Markdown/CSV 可加 `--keep-extra`。内容均为简体中文。标准 run 会安全复制到相邻 `./outputs`；临时或任意目录不会自动发布。
...
**Caching, accuracy, and speed (automatic):** repository details, Trending details, README, and latest release use independent 24 h timestamps so one cache route cannot revive stale data from another. Authenticated mode enriches every retained candidate before choosing Top N; anonymous mode skips ranking-critical release/Issue/PR calls and enriches a bounded buffer to protect quota. Detail enrichment runs with limited parallelism when authenticated. Cache hits are printed and recorded in `collection.detail_cache_hits`.
...
The command validates project coverage and JSON types (one_liner, four detail blocks, fact URLs), applies the data-metric ranking, updates `result.json`, and writes the interactive card report `report.html` as the **only** run-local output file (pass `--keep-extra` for Markdown/CSV). Standard script-created runs are also mirrored to the adjacent `outputs/` directory; use `--no-publish` to disable this or `--publish-dir` to choose a destination. Arbitrary/temp run directories never auto-publish.
